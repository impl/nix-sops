...
let
  ...
  in self.lib.mkShellAndIfList (flatten [
    ...
    (map (source: "${pkgs.sops}/bin/sops --config /dev/null --decrypt ${optionalString (source.outputType != null) "--output-type ${escapeShellArg source.outputType}"} ${optionalString (source.key != null) "--extract ${escapeShellArg source.key}"} ${escapeShellArg (self.lib.copySourceToStoreSanitized source)} >>${targetI}") secret.sources)
